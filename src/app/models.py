...
class FeedAccessToken(db.Model):  # type: ignore[name-defined, misc]
    ...
    feed_id = db.Column(db.Integer, db.ForeignKey("feed.id"), nullable=True)  # NULL for combined feed tokens
...
class Post(db.Model):  # type: ignore[name-defined, misc]
    ...
    )  # remote download URL, not podly url (not unique - some feeds reuse URLs)
    title = db.Column(db.Text, nullable=False)
