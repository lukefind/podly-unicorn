          ...
          {isLoading ? (
            ...
            <div>
              {allFeeds.map((feed, idx) => (
                <div 
                  key={feed.id}
                  className={`px-4 sm:px-6 py-2.5 sm:py-3 transition-colors ${
                    idx > 0
                      ? isOriginal
                ...
                >
                  <div className="flex items-center gap-3">
                    {feed.image_url ? (
                      ...
                      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center flex-shrink-0 ${isOriginal ? 'bg-blue-800/40' : 'bg-gray-200 dark:bg-gray-700'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-xs ${isOriginal ? 'text-blue-300/70' : 'text-gray-500 dark:text-gray-400'}`}>{feed.posts_count} episodes</p>
                    </div>
                    ...
                    <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
                      {feed.is_subscribed ? (
                        <>
                          <button
                            onClick={() => handleTogglePrivacy(feed.id, feed.is_private || false)}
                            disabled={subscribeMutation.isPending}
                            className={`p-1.5 rounded-lg transition-colors ${
                              feed.is_private
                                ? isOriginal ? 'bg-blue-700/60 text-blue-200' : 'bg-gray-700 text-white'
                                : isOriginal ? 'text-blue-300/60 hover:text-blue-200 hover:bg-blue-800/40' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700'
                            }`}
                            title={feed.is_private
                        ...
                        <div className="flex items-center gap-0.5">
                          <button
                            ...
                            disabled={subscribeMutation.isPending}
                            className={`px-3 py-1.5 text-xs font-medium rounded-l-lg disabled:opacity-50 ${
                              isOriginal
                          ...
                          <button
                            ...
                            disabled={subscribeMutation.isPending}
                            className={`px-1.5 py-1.5 text-xs font-medium rounded-r-lg disabled:opacity-50 ${
                              isOriginal
                  ...
                  <div className="flex sm:hidden items-center gap-1.5 mt-1.5 pl-[52px]">
                    {feed.is_subscribed ? (
                      <>
                        <button
                          onClick={() => handleTogglePrivacy(feed.id, feed.is_private || false)}
                        ...
                        <button
                          onClick={() => handleUnsubscribe(feed.id)}
                          disabled={unsubscribeMutation.isPending}
                          className={`px-2.5 py-0.5 text-xs font-medium rounded-md disabled:opacity-50 ${
                            isOriginal
                              ? 'bg-blue-500/80 text-white hover:bg-blue-400/80'
                              : 'bg-purple-600 text-white hover:bg-purple-700'
                          }`}
                      ...
                      <div className="flex items-center gap-0.5">
                        <button
                          onClick={() => handleSubscribe(feed.id, false)}
                          disabled={subscribeMutation.isPending}
                          className={`px-2.5 py-0.5 text-xs font-medium rounded-l-md disabled:opacity-50 ${
                            isOriginal
                              ? 'bg-blue-800/50 text-blue-200 hover:bg-blue-700/60'
                              : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'
                          }`}
                        ...
                        <button
                          onClick={() => handleSubscribe(feed.id, true)}
                          disabled={subscribeMutation.isPending}
                          className={`px-1.5 py-0.5 text-xs font-medium rounded-r-md disabled:opacity-50 ${
                            isOriginal
                              ? 'bg-blue-800/40 text-blue-300 hover:bg-blue-700/50'
                              : 'bg-gray-300 dark:bg-gray-600 text-gray-600 dark:text-gray-400 hover:bg-gray-400 dark:hover:bg-gray-500'
                          }`}
            ...
            <div className="text-center py-8 px-4">
              <p className={`${isOriginal ? 'text-blue-200' : 'text-gray-500 dark:text-gray-400'}`}>No podcasts available</p>
              <p className={`text-sm mt-1 ${isOriginal ? 'text-blue-300/60' : 'text-gray-400 dark:text-gray-500'}`}>Ask an admin to add some podcasts first</p>
            </div>
          )}
        </div>
      </div>
